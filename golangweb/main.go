...
func ShowUserFunc(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		for _, usr := range alUsrs.Users {
			if IsValueInSlice(allID, id) != true {
...
func main() {
	http.HandleFunc("/addnewuser/", AddNewUserFunc)
	http.HandleFunc("/notsucceded", NotSucceded)

	http.HandleFunc("/deleted", DeletedFunc)
	http.HandleFunc("/deleteuser/deleted", DeleteUserFunc)
	http.HandleFunc("/deleteuser/",DeleteUserServe)
	http.HandleFunc("/deleteuser/notsuccededdelete",NotSuccededDelete)

	http.HandleFunc("/",IndexFunc)

	http.HandleFunc("/showuser/show",ShowUserFunc)
	http.HandleFunc("/showuser/",ShowUser)
	http.HandleFunc("/showuser/notsuccededshow/",NotSuccededShow)
